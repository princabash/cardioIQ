...
module.exports = async function handler(req, res) {
  ...
  try {
    const { stash_id, payment_id, tier } = req.body || {};
    if (!stash_id || !payment_id || !tier) {
      return res.status(400).json({ error: 'Missing stash_id, payment_id, or tier' });
    }
    ...
    if (!process.env.DODO_PAYMENTS_API_KEY) {
      return res.status(500).json({ error: 'DODO_PAYMENTS_API_KEY not set' });
    }
    ...
    if (!SUCCESS_STATUSES.includes(paidStatus)) {
      return res.status(402).json({ error: 'Payment not confirmed (status: ' + paidStatus + ')' });
    }
    ...
    if (paidAmount != null && Math.round(paidAmount) < TIER_PRICES[tier]) {
      return res.status(402).json({ error: 'Paid amount does not match requested tier' });
    }
    ...
    if (!raw) return res.status(404).json({ error: 'Intake data not found or already used' });
